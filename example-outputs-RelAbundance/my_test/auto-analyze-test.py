...
meta = DownloadHelper.combine_metadata_files(
    ['cd_young.tsv', 'cd_old.tsv', 'uc_young.tsv', 'uc_old.tsv'])
...
# results.to_csv('test_metadata.tsv', sep='\t', index=False)

# with open('test_metadata.tsv', 'a') as meta:
#     for counter, file in enumerate(glob.glob("*.tsv")):
...
bar = qhelper.create_barplot(
    input_feat_table='feat-table.qza',
    input_tax_class=tc,
    metadata_filepath=meta,
    output_filepath='taxa-bar-plots.qzv')
