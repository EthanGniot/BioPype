...
\subsection{Creating a New Virtual Environment}
...
\subsection{Installing packages}
    ...
    \begin{enumerate}
        ...
        \item In the ``Search Packages" box, enter ``biopython". The search should return a package named ``biopython". Select the checkbox to the left of the name. \todo{add reference to the anaconda-search-pack figure.} 
        \begin{itemize}
            \item A pair of green and red boxes (reading ``Apply" and ``Clear", respectively) will appear in the bottom-right of the window once the package is selected. Do not click these just yet. \missingfigure{replace this with ``anaconda-search-pack".}
        \end{itemize}
        \item Use the search bar to find and select the other packages listed in \autoref{tab:software}. Once all packages have been selected, click the green ``Apply" button in the bottom right corner of the window, then select ``Apply" again within the ``Install Packages" window that appears. (\todo{reference the anaconda-install-pack figure.}) Anaconda will now install the selected packages.
        \begin{itemize}
        ...
        \end{itemize}
        \missingfigure{Replace with anaconda-install-pack image.} 
        ...
        \begin{enumerate}
            ...
            \item Wait for the terminal window to finish opening. You'll know it's finished when you see \todo{show image of what the prompt looks like when it's done initializing.}
